...
public class PedidosService {
	...
	@DELETE
	@Produces("application/json")
	public Response deletePedido(@PathParam("id") int pid) {


		if (pid > 0) {
			try {
			return Response.status(200).entity(PedidosManager.getInstance().deletePedido(pid)).build();
			} catch (Exception e) {
				System.out.println("Exception: "+e.getMessage());
				return Response.status(500).entity(new StatusMessage(500, "Error en el Server")).build();
			}
		}else {
			return Response.status(404).entity(new StatusMessage(404, "Aqui no hay nada")).build();
		}
	}
